...
export const MenusPage: React.FC = () => {
  ...
  const handleAddMenu = (menu: Omit<Menu, 'id' | 'created_at'>) => {
    try {
      ...
      setError(err instanceof Error ? err.message : '메뉴 등록에 실패했습니다.');
    }
  };
  ...
  const handleAddRecipe = (recipe: Omit<Recipe, 'id'>) => {
    try {
      ...
      setError(err instanceof Error ? err.message : '레시피 등록에 실패했습니다.');
    }
  };
  ...
  const handleEditRecipe = (recipe: Omit<Recipe, 'id'>) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : '레시피 수정에 실패했습니다.');
    }
  };
